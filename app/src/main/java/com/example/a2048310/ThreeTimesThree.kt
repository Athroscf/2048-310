...
import com.example.a2048310.Tiles3.Companion.movimientosRestantes
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase
import kotlinx.android.synthetic.main.activity_three_times_three.*

class ThreeTimesThree : AppCompatActivity() {

    companion object {
        private const val GAME_KEY = "2048_GAME_KEY"
    }

    private lateinit var game: Tiles3
    private var x1 = 0f
    private var y1 = 0f
    private var cells = IntArray(9)
    private lateinit var cronometer: Chronometer

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_three_times_three)
        ...
        if (savedInstanceState != null) {
            val tmpGame :Tiles3? = savedInstanceState.getSerializable(GAME_KEY) as Tiles3
            if (tmpGame != null) {
                this.game = tmpGame
                game.rePoblar()
            }
        } else this.game = Tiles3()
        dibujarCuadros()
...
*/
    ...
    private fun laTransicion(trans: Transition) {
